...
class ShapeProcessor:
    ...
    def load_dino_directory(self, h5py_files_path: str) -> DinoShapeData:
        ...
        try:
            ...
            feature_point_cloud_path = os.path.join(h5py_files_path, 'feature_models', 'dinov2_vits14') # dinov2_vits14 # dinov2_vitl14_reg


            category_name = os.path.basename(h5py_files_path)
            ...
            def load_point_cloud_features(npz_path):
                """
                Load the saved point cloud with features
                """
                data = np.load(npz_path)
                
                # Option 1: Load concatenated data
                if 'point_cloud_features' in data:
                    point_cloud_with_features = data['point_cloud_features']
                    positions = point_cloud_with_features[:, :3]
                    features = point_cloud_with_features[:, 3:]
                
                # Option 2: Load separate arrays
                elif 'positions' in data and 'features' in data:
                    positions = data['positions']
                    features = data['features']
                
                return point_cloud_with_features
            ...
            for i, obj_folder in enumerate(tqdm(os.listdir(feature_point_cloud_path), desc=f"Processing {category_name}")):
                
                # object_name = f"{category_name}_{i}"  # e.g., chair_1, chair_2, etc.
                object_name= obj_folder.split('.')[0]
                print("show object_name_sha256:",object_name)
                # print("show object_name:",object_name)
                # exit(0)
                npz_file_path = os.path.join(feature_point_cloud_path, obj_folder)




                # print(len(category_data))
                print("h5_file_path:",npz_file_path)
                ...
                if os.path.exists(npz_file_path):
                    ...
                    feature_point_cloud = load_point_cloud_features(npz_file_path) # entry point cloud 
                    print("show feature_point_cloud dim:",feature_point_cloud.shape) #  (4096, 387), (4096, 1024+3)    
                    # exit(0)
                    ...
                    kpts_data.append(feature_point_cloud)
...
class ShapeCanonicalizeGUI:
    ...
    def setup_control_panel(self, parent):
        ...
        ttk.Button(
            ...
            command=lambda: self.load_dino_directory(str(self.dino_directory_path.get()))
...
# select_res_save_directory
    ...
    def load_dino_directory(self, dino_path:str ):
        ...
        if dino_path:
            try:
                ...
                self.processor.dino_data = self.processor.load_dino_directory(dino_path)
    ...
    def setup_cluster_controls_with_selection(self, parent, cluster_id):
        ...
        delete_button = ttk.Button(shape_mgmt_frame, text="Delete Shape", 
                          command=lambda: self.delete_selected_shape(cluster_id))
    ...
    def perform_shape_deletion(self, shape_idx):
        ...
        try:
            ...
            # Update original coordinates if they exist
            ...
            # Clean up individual shape rotations
            ...
            self.update_cluster_data_after_deletion(shape_idx)
    ...
    def update_cluster_data_after_deletion(self, deleted_shape_idx):
