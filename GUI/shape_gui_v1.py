...
class ShapeCanonicalizeGUI:
    ...
    #     """Setup controls specific to a cluster tab"""
    #     # Cluster info
    ...
    #     ttk.Radiobutton(view_frame, text="Center Only", variable=cluster_view_var, value="center_only", command=lambda: self.update_cluster_tab_visualization(cluster_id)).pack(anchor=tk.W)
    #     ttk.Radiobutton(view_frame, text="All Shapes in Cluster", variable=cluster_view_var,  value="all_shapes", command=lambda: self.update_cluster_tab_visualization(cluster_id)).pack(anchor=tk.W)
        

    #     # Rotation controls specific to this cluster
    #     rotation_frame = ttk.LabelFrame(parent, text="Rotation Controls", padding=10)
    #     rotation_frame.pack(fill=tk.X, pady=(0, 10))
    ...
    #     ttk.Button(action_frame, text="Apply to All Shapes", 
    #             command=lambda: self.apply_rotation_to_cluster_shapes(cluster_id)).pack(fill=tk.X, pady=2)
    ...
    #     cluster_status_text = tk.Text(status_frame, height=4, width=25)
    ...
    def add_coordinate_system_to_ax(self, ax):
        """Add coordinate system with TRUE unit length for all axes"""
        # Get the current axis limits
        xlim = ax.get_xlim()
        ylim = ax.get_ylim()
        zlim = ax.get_zlim()
        
        # CRITICAL FIX: Use the SAME length for all three axes
        ...
        # ALL axes use the same arrow length (percentage of the largest range)
        arrow_scale = 0.15  # 15% of the largest range
        arrow_length = max_range * arrow_scale  # SAME for X, Y, Z
        ...
        print(f"  Arrow length (ALL axes): {arrow_length:.3f}")
        print(f"  Unit length verification: X={arrow_length:.3f}, Y={arrow_length:.3f}, Z={arrow_length:.3f}")
        
        # Position origin at a corner of the plot
        origin_x = xlim[0] - x_range * 0.1  # 10% outside left edge
        origin_y = ylim[0] - y_range * 0.1  # 10% outside bottom edge  
        origin_z = zlim[0] - z_range * 0.1  # 10% outside front edge
        
        # Define axes with IDENTICAL arrow lengths
        axes_data = [
            # X-axis: moves arrow_length in X direction
            ([origin_x, origin_x + arrow_length], [origin_y, origin_y], [origin_z, origin_z], 'red', 'X'),
            # Y-axis: moves arrow_length in Y direction
            ([origin_x, origin_x], [origin_y, origin_y + arrow_length], [origin_z, origin_z], 'green', 'Y'), 
            # Z-axis: moves arrow_length in Z direction
            ([origin_x, origin_x], [origin_y, origin_y], [origin_z, origin_z + arrow_length], 'blue', 'Z')
        ]
        ...
        for x_coords, y_coords, z_coords, color, label in axes_data:
            ...
            # Use consistent offset for all labels
            label_offset = arrow_length * 0.1  # 10% of arrow length
            label_x = end_x + (label_offset if label == 'X' else 0)
            label_y = end_y + (label_offset if label == 'Y' else 0)
            label_z = end_z + (label_offset if label == 'Z' else 0)
            
            ax.text(label_x, label_y, label_z, label,
                    color=color, fontsize=14, fontweight='bold')
        
        # Add origin marker for reference
        ax.scatter([origin_x], [origin_y], [origin_z], 
                color='black', s=50, alpha=0.8, marker='o')
        
        # Verification: Calculate actual vector lengths
        x_vector = [arrow_length, 0, 0]  # X-axis vector
        y_vector = [0, arrow_length, 0]  # Y-axis vector  
        z_vector = [0, 0, arrow_length]  # Z-axis vector
        
        x_length = np.linalg.norm(x_vector)
        y_length = np.linalg.norm(y_vector)
        z_length = np.linalg.norm(z_vector)
        
        print(f"  VERIFICATION - Actual vector lengths:")
        print(f"    X-axis vector length: {x_length:.6f}")
        print(f"    Y-axis vector length: {y_length:.6f}")
        print(f"    Z-axis vector length: {z_length:.6f}")
        print(f"    All equal? {np.allclose([x_length, y_length, z_length], arrow_length)}")
