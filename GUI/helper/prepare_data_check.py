...
def process_category(category_path, output_path):
    ...
    for i, obj_folder in enumerate(tqdm(object_folders, desc=f"Processing {category_name}")):
        ...
        print("output_path:",output_path)# /home/lei/Documents/Dataset/Canonicalization/G-objaverse_h5py_files/chairr
    ...
    #----------------------------------------------------------------》》》 print("kpts_data shape:",kpts_data.shape) # (16, 2048, 387) # 3+384
    # exit(0)
    ...
    pcd.colors = o3d.utility.Vector3dVector(all_colors)
    o3d.visualization.draw_geometries([pcd])
    # save the tsne-knn results
    category_res= os.path.join(category_path,f"{category_name}.pcd")
    o3d.io.write_point_cloud(category_res, pcd)
    # exit(0)
...
def run():
    ...
    dir = '/home/lei/Documents/Dataset/Canonicalization'
    data_root = dir+'/G-objaverse'  # Root directory containing category folders
    
    data_dir = dir+ '/G-objaverse_h5py_files'  # Directory for output h5py files
    # data_dir = dir+ '/G-objaverse_h5py_files_rotated'  # Directory for output h5py files
    
    
    feature_model_dir = dir+ '/G-objaverse_feature_model'
    feature_image_dir = dir+ '/G-objaverse_feature_image'
