...
if __name__=='__main__':
    ...
    for category, id_url_map in category_id_to_url.items():
        ...
        for shape_id, url in id_url_map.items():
            ...
            res_folder= os.path.join(category_dir, subfolder)

            print("see existing res",res)
            print("see res_folder res",res_folder)


            # exit(0)
      
            if os.path.exists(res) or os.path.exists(res_folder) :
                print("see existing res",res)
                counter+=1
                continue
    ...
    # conda activate freereg && export LD_LIBRARY_PATH=$CONDA_PREFIX/lib:$LD_LIBRARY_PATH
