...
def process_category(category_path, output_path, feature_model_path, feature_vis_path,id_info):
    ...
    for i, obj_folder in enumerate(tqdm(object_folders, desc=f"Processing {category_name}")):
        ...
        # object_path= os.path.join(object_path, 'campos_512_v1')
        print("show object_path:",object_path)
        subfolders = sorted([f for f in os.listdir(object_path) if os.path.isdir(os.path.join(object_path, f))])
        object_name = f"{category_name}_{i}"  # e.g., chair_1, chair_2, etc.
        
        # print("show subfolders:",subfolders)
        # exit(0)
        ...
        # print("show object_data['feature_points'].shape:",object_data['feature_points'].shape)
        # exit(0)
    return output_path
...
def run():
    ...
    # category='cellphone_lvis' # cellphone_lvis # chair_compare
    base_dir = "/media/lei/Extreme SSD/Canonicalization/categories20"
